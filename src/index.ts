import { defaultJsonSerializer } from "./utils/defaultJsonSerializer.js";
import { HeadersInstanceToPlainObject, uppercase } from "./utils/helpers.js";
import {
...
} from "./utils/parseArgs.js";
import { resolveRequestDocument } from "./utils/resolveRequestDocument.js";
import type {
...
type BuildRequestConfigParams<V> =
  | BuildRequestConfigParamsSingle<V>
  | BuildRequestConfigParamsBatch<V>;

/**
 * Create query string for GraphQL request
 */
const buildRequestConfig = <V extends Variables>(
  params: BuildRequestConfigParams<V>
): string => {
    ...
    if (params.variables) {
      search.push(
        `variables=${encodeURIComponent(
          params_.jsonSerializer.stringify(params_.variables)
        )}`
      );
    }
  ...
  async <V extends Variables>(params: RequestVerbParams<V>) => {
    ...
    if (middleware) {
      const result = await Promise.resolve(
        middleware({ ...init, url, operationName, variables })
      );
...
const parseBatchRequestsArgsExtended = (
...
export { GraphQLWebSocketClient } from "./utils/graphql-ws.js";
export { resolveRequestDocument } from "./utils/resolveRequestDocument.js";
export {
  BatchRequestDocument,
  batchRequests,
  BatchRequestsExtendedOptions,
